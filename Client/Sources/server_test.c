...
int main(void)
{
    ...
    int sock, i, j, slen = sizeof(si_other);
    char buf[BUFLEN];
    struct client clients[2]; // 10 clients. Notice that we're not doing any bound checking.
    int n = 0;
    ...
    while (1) {
        ...
        for (i = 0; i < n; i++) {
            ...
            for (j = 0; j < n; j++) {
                ...
                if (sendto(sock, &clients[j], 6, 0, (struct sockaddr *) (&si_other), slen) == -1)
                    diep("sendto");
            }
        }
